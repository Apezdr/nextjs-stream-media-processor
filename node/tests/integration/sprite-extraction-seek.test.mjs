/**
 * Seek-mode sprite extraction against the real ffmpeg: one ffmpeg per
 * timestamp, composed with sharp, progress counted from completed frames.
 * Companion to sprite-extraction-linear.test.mjs; the strategy is fixed
 * when sprite.mjs loads, so each mode gets its own file. Skipped when
...
process.env.SPRITE_SEEK_MODE = 'accurate';
...
describeWithTools('seek sprite extraction (real ffmpeg)', () => {
  const CLIP_SECONDS = 32;
  const INTERVAL = 5;
  let dir;
  let clip;
