...
router.post('/subtitles/save', authenticateUser, requireAdmin, async (req, res) => {
    try {
        ...
        // Auto-generated tracks are saved next to the source SRT under a separate
        // `.auto.srt` filename so editing them doesn't shadow or replace any
        // human-authored `.srt` for the same language.
        const isAutoGenerated = /\bauto\s+generated\b/i.test(language);
        const variantSuffix = `${isAutoGenerated ? '.auto' : ''}${isHearingImpaired ? '.hi' : ''}`;
