...
export async function scanMovies(db, dirPath, prefixPath, basePath, langMap, currentVersion, isDebugMode, downloadTMDBImages) {
  ...
  // Per-pass failure accounting (SI-P0, "failed ≠ absent"). A directory that
  // throws while being processed is retained exactly as it was and re-examined
  // next tick; it is NEVER read as departed. Collected here so the pass can
  // summarise at the end instead of rejecting the whole Promise.all (which
  // used to skip the removal loop below AND — via app.mjs runGenerateList's
  // sequential orchestration — the TV scan and autoSync for that tick).
  const failedMovies = [];
  let scannedDirs = 0;
  ...
  await Promise.all(
    dirs.map((dir, index) => movieLimit(async () => {
      ...
      // "failed ≠ absent" (SI-P0): mark this title as PRESENT before any I/O.
      // Everything below can throw (readdir on a folder deleted mid-scan,
      // calculateDirectoryHash on an unreadable entry, a DB hiccup); the catch
      // at the end of this closure swallows that, so the delete MUST already
      // have happened or a transient blip would leave the name in the removal
      // set and the loop after Promise.all would delete a live movie. A folder
      // that is genuinely gone is removed on the NEXT tick, when the root
      // listing no longer carries it (tv-scanner.mjs has always ordered it this
      // way).
      ...
      try {
        ...
        // (warn-and-continue) so the title is still PROCESSED this pass with
        // fail-closed defaults, rather than retained-and-skipped by the SI-P0
        // per-title catch. Fail CLOSED on the freeze flag (§4.2): an
        ...
        // Isolation, not suppression: the error is logged with its code and the
        // title is counted for the per-pass summary. Nothing here may rethrow —
        // one unreadable or vanished folder must not reject Promise.all.
        //
        // "Retained" means not removed — the row is whatever it was when the
        // throw happened: untouched for a pre-save failure; as saved for a
        // failure after saveMovie (in which case the next tick fast-skips on
        // the stored directory_hash and any stale hash row is left to SI-P3's
        // reconcile). The stack is attached only for non-fs errors (no code):
        // fs errors are self-describing, code bugs are not.
        const message = err?.message ?? String(err);
        failedMovies.push({ name: dirName, code: err?.code, message });
        logger.error('scanner: movie directory failed this pass — retained (not removed), re-examined next tick', {
          'media.name': dirName,
          'media.type': 'movie',
          'error.code': err?.code,
  ...
  // Per-pass summary of isolated failures (SI-P0). One line, not one per title,
  // so SigNoz can distinguish "a folder is broken" from "the scan is broken":
  // a partial failure is a WARN; every directory failing is library-wide
  // (dead mount after the root listing, a code regression inside the closure)
  // and is an ERROR with a stable prefix — alertable, but deliberately NOT a
  // rethrow, which would again skip the TV scan and autoSync for the tick
  // (app.mjs runGenerateList). SI-P1 adds the fraction guard on removals.
  if (failedMovies.length > 0) {
    const libraryWide = failedMovies.length === scannedDirs;
    const summary = {
      'media.type': 'movie',
      scannedCount: scannedDirs,
      failedCount: failedMovies.length,
      failed: failedMovies.slice(0, 20).map(f => `${f.name} (${f.code || f.message})`),
    };
    if (libraryWide) {
      logger.error(`scanner: every movie directory failed this pass (${failedMovies.length} of ${scannedDirs}) — library-wide fault, nothing removed`, summary);
    } else {
      logger.warn(`scanner: ${failedMovies.length} of ${scannedDirs} movie directories failed this pass and were retained`, summary);
    }
  }
  ...
  //
  // Observability + isolation (SI-P0): the removal path used to log nothing at
  // info level, so a departure left no trail in SigNoz; and one failing cascade
  // rejected scanMovies outright. Each title's cascade is now isolated — a
  // failure is logged and the remaining titles still go. The title row is
  // deleted LAST: the removal set is rebuilt from the movies table each tick,
  // so keeping the row until the dependent rows are gone makes the row the
  // retry anchor — a half-applied cascade is retried next tick instead of
  // stranding orphan hash/cooldown rows. SI-P2 makes the cascade one
  // transaction; SI-P1 adds the zero-dirs / fraction guard in front of this.
  if (existingMovieNames.size > 0) {
    logger.info(`scanner: removing ${existingMovieNames.size} movie${existingMovieNames.size === 1 ? '' : 's'} no longer on disk`, {
      'media.type': 'movie',
      removedCount: existingMovieNames.size,
      removed: [...existingMovieNames].slice(0, 20),
    });
  }
  for (const movieName of existingMovieNames) {
    try {
      ...
      logger.error('scanner: movie removal cascade failed — title row kept as retry anchor, retried next tick', {
