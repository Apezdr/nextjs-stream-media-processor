...
import {
  withFFmpegSpan,
  withClipGenerationSpan,
  withTranscodeSpan,
} from "./videoTracer.mjs";
 ...
 */
function mergeTranscodeSettings(
  encoderConfig,
  profile,
  userOverrides,
  includeAudio = true,
...
export async function generateFullTranscode(
  ...
  return withTranscodeSpan(
    {
      inputPath,
      outputPath,
      codec: targetCodec,
      operation: "transcode",
      hardwareAcceleration: targetCodec.includes("vaapi")
        ? "vaapi"
        : targetCodec.includes("nvenc")
          ? "nvenc"
          : "none",
    },
    async () => {
      try {
        ...
        if (targetCodec === "vp9_vaapi") {
          ...
          logger.warn(
            `Unsupported codec "${targetCodec}". Falling back to libx264.`,
          );
          selectedEncoderConfig = libx264;
        }
        ...
        if (!encoderProfiles || !encoderProfiles[profileName]) {
          throw new Error(
            `Profile "${profileName}" not found for encoder "${targetCodec}".`,
          );
        }
        ...
        const mergedArgs = mergeTranscodeSettings(
        ...
        if (doTwoPass) {
          logger.info(
            `Two-pass encoding requested for profile: ${profileName}. Running pass 1 & pass 2...`,
          );
          ...
          if (selectedEncoderConfig.vaapi_device) {
            ffmpegArgs.push(
              "-vaapi_device",
              selectedEncoderConfig.vaapi_device,
            );
          }
          ...
          // Log and execute
          logger.info(
            `Executing SINGLE PASS FFmpeg command: ffmpeg ${ffmpegArgs.join(" ")}`,
          );
          await executeFFmpeg(ffmpegArgs);
          ...
          logger.info(
            `Successfully transcoded video (single-pass) to: ${outputPath}`,
          );
        }
      } catch (error) {
  ...
  return withClipGenerationSpan(
    {
      ...
      startTime: start,
      duration: end - start,
      codec: selectedEncoderConfig.encoder,
      operation: "clip",
    },
    async () => {
      ...
      try {
        ...
        logger.info(
          `Executing FFmpeg extract command: ffmpeg ${extractArgs.join(" ")}`,
        );
        await executeFFmpeg(extractArgs);

        // Step 2: Select profile for the encoder
        if (
          !selectedEncoderConfig.profiles ||
          !selectedEncoderConfig.profiles[profileName]
        ) {
          throw new Error(
            `Profile "${profileName}" not found for encoder "${selectedEncoderConfig.codec}".`,
          );
        }
        ...
        if (doTwoPass) {
          logger.info(
            `Two-pass clip encoding requested for profile: ${profileName}. Running pass 1 & pass 2...`,
          );
          await runTwoPassTranscode({
          ...
          // Log and execute
          logger.info(
            `Executing SINGLE PASS clip encode: ffmpeg ${ffmpegArgs.join(" ")}`,
          );
          await executeFFmpeg(ffmpegArgs);
          ...
          logger.info(
            `Successfully generated clip (single-pass) at: ${cachedClipPath}`,
          );
        }
