...
**Terminology.** One term per concept, used consistently: **freeze** (the `update_metadata: false` switch — see Appendix C), **pristine base** (the planned raw pre-override TMDB snapshot), **ground truth**, and **derived state**.
...
| 2 | Read + parse an existing file | `tv_shows.metadata` (mirrors `metadata.json`), `backdrop_focal` (mirrors `tmdb.config`), `metadata_hashes` rows (recomputed from row data + `metadata.json` content by `generateMovieHashes()` / `generateTVShowHashes()` in `node/sqlite/metadataHashes.mjs`) |
...
The filesystem/SQLite duality omits a real tier: this backend also owns MongoDB state (connection helpers and provisioning in `node/database.mjs`). It has three distinct sub-kinds with different risk profiles:

1. **Non-derivable identity/admin state.** The `Users` database (`account` / `user` collections, Better Auth schema) links Discord OAuth accounts to admin users; `getAdminByDiscordId()` in `node/database.mjs` reads it to authorize Discord bot commands. This repo only reads these collections — they are written by the frontend — but nothing in either repo can regenerate them.
...
3. **Dead legacy collections.** `initializeMongoDatabase()` in `node/database.mjs` provisions `Media.Movies`, `Media.TV`, `Media.PlaybackStatus`, and `Cache.cacheEntries` on every startup (and `initializeIndexes()` builds indexes on all four), yet no code in this repo reads or writes any of them. Decided 2026-07-07 (M-3): this dead provisioning is slated for deletion — not yet implemented; until then, do not mistake the provisioning code for evidence that this backend uses those collections.
...
> **Why the planned pristine-base snapshot is a SQLite column, not a file.**
> Decided 2026-07-07 — not yet implemented: a "pristine base" snapshot of TMDB's raw, pre-override response will be stored as a new SQLite column rather than a new on-disk file. This is a direct application of §1's principle, not an afterthought: the snapshot is a cache of TMDB's raw response — a derived artifact, so it belongs in the derived store — while ground truth already has a home in `metadata.json`. Adding a second ground-truth file would have created a second thing to back up, reconcile, and reason about; adding a column just makes the derived store richer. (Full planned lifecycle: the pristine-base entry at the end of §3.)
...
**Known sharp edges (current behavior):** `/rescan/tmdb` in `node/app.mjs` has no authentication while triggering a full-library `forceRefresh` (`runDownloadTmdbImages({ fullScan: true })`) — the most privileged mutation on the surface (open item A-1; see §4.9). The config-update endpoint is a full-file replace with no server-side merge — deliberate and load-bearing (A-2; see the warning block in §4.6). The manual `/media/scan` route only scans movies — it calls `generateListMovies` with no TV equivalent (S-3; see §4.7).
...
| Images-missing | `missingImages` — any of poster/backdrop/logo absent or stale on disk | **none** for TV — retried every scan tick; for movies, retried only when `dirHashChanged` forces processing | `tv-scanner.mjs` `processShowAssets()` (gate computed pre-skip every tick); `movie-scanner.mjs` `checkTMDBImagesNeeded()` (reached only past the unchanged-hash early return) |
| Metadata-missing | `missingMetadata` — `metadata.json` absent, or older than `tmdb.config` | **24 h** via `missing_data_media` (`RETRY_INTERVAL_HOURS` in `scanner-repository.mjs`) — the 24 h cycle is effective for TV; for movies the gate is unreachable while the directory hash is stable | scanner gate + repository bookkeeping |
...
| Frozen | `update_metadata: false` in `tmdb.config` | n/a — generator early-returns | `metadataGenerator.mjs` `generateForShow()` / `generateForMovie()` via `isUpdateAllowed()` |
| No-TMDB-match | not detectable — search failure throws a generic `Error` | inherits the metadata cooldown (24 h cycle for TV; movies retry only on directory change) | nowhere (see G‑1 below) |
| Removed | directory gone from disk | — | scanner `removeMovie()` / `removeTVShow()` |

Details per gate:

**Images-missing (no cooldown).** The scanner deliberately exempts image failures from the 24 h cooldown on the theory that image-CDN failures are transient: `runDownloadTmdbImagesFlag = dirHashChanged || missingImages || (missingMetadata && metadataGateAllowsRetry)` — `missingImages` is unconditioned. How often that formula is even evaluated differs by scanner. The TV scanner computes it on every tick — its fast-skip was deliberately restructured to run *after* the gate, precisely because the old movie-style hash-skip suppressed image-only retries (the comment in `tv-scanner.mjs` `scanTVShows()` records this). The movie scanner still has the pre-restructure shape: an unchanged-hash movie returns early before `checkTMDBImagesNeeded()`, and a permanently missing image never moves the directory hash, so after the first failed pass saves the post-attempt hash the movie is skipped on every subsequent tick — the image is retried only when the directory contents change. Consequence worth knowing: a **frozen** TV show with a permanently missing image re-invokes the generator every scan tick forever; the generator's freeze check turns each invocation into a no-op, but the invocation cost recurs unboundedly (tracked as R‑2). A frozen movie in the same state is re-offered only on a directory change.

**Metadata-missing (24 h cooldown).** The cooldown row in `missing_data_media` is stamped **on attempt, not on failure** — `markMediaAsMissingData()` runs *before* the download attempt, and `clearMissingMediaData()` runs after only if *both* metadata and images are then present. A same-tick success marks then clears in one pass, so a row's presence does not mean "currently failing" at any instant mid-scan (tracked as R‑1). Partial recovery (metadata fetched, an image still failing) intentionally leaves the row in place. The 24 h *retry cycle* itself is real only for TV: the movie scanner consults `metadataGateAllowsRetry` only when the movie is processed at all, and the unchanged-hash early return short-circuits before that check — so a movie's failed metadata lookup is effectively retried once per directory-content change, with the `missing_data_media` row persisting but its 24 h gate unreachable while the hash is stable.
...
**Frozen (`update_metadata: false`).** The freeze is enforced in exactly one layer — the generator — as the *first* effective check in both `generateForShow()` and `generateForMovie()`, before any TMDB fetch. The scanner's own gate computation stays freeze-blind by design. Two nuances of the freeze as shipped:
...
**No-TMDB-match.** When no `tmdb_id` is pinned and the title search returns nothing (after a retry with the `(year)` suffix stripped), `tmdb.mjs` `fetchComprehensiveMediaDetails()` throws a generic `Error("No results found …")`. The generator catches it and returns `{ success: false, error }`; the scanner ignores the returned reason entirely and re-checks only disk state. Result: at the scanner's observation layer, "operator froze it", "TMDB has no such title", and "TMDB was down for five minutes" are **indistinguishable** — all three leave `metadata.json` missing and the same cooldown row in place — cycling every 24 hours for a TV show, and re-attempted only on the next directory-content change for a movie. Decided 2026‑07‑07 (G‑1a/G‑1b, Branch 3, **not yet implemented**): the no-match case becomes a typed error, with a `reason` (`'no-match'` vs `'transient-error'`) plumbed through the scanner-facing return contract so the two cases stop collapsing into one.
...
**Known gap (R‑3, Branch 3):** rows whose episode exhausts the 90-day window are never deleted — `isEpisodeBackfillDue()` simply returns `false` forever, so the row never resolves and never gets pruned. Bounded by real episode count, but permanent clutter for genuinely-unfillable episodes. Decided 2026‑07‑07 to address under R‑3 (**not yet implemented**).
...
| Give-up | never | 90 days after air date (row then orphaned — R‑3) |
...
- **Id-pinned.** Either the operator writes `tmdb_id`, or the scanner-driven generator does it itself: when a title search succeeds, `updateTmdbConfigWithId()` persists the found id. Verified in current code: this function is still an **add-only one-way ratchet** — it writes `tmdb_id` only `if (!config.tmdb_id)` and never overwrites an existing one. The generator can therefore pin an id but can never change or unpin it; correcting a bad auto-match is always a manual config edit. Note the "already up to date" branch of `generateForShow()` also performs this search-and-pin (it needs an id for season processing) — an id can get pinned without any show-level metadata validation.
...
- **Composite hazard — deleting `metadata.json` while frozen never gets it back (no overrides case).** Verified trace: the scanner sees `missingMetadata`, marks the 24 h cooldown, and invokes the generator; the generator's freeze check runs **before** any fetch or write path, `_applyOverridesWhileFrozen()` finds no overrides and returns `applied: false`, and the generator exits with `reason: 'updates-disabled'` having touched nothing. The scanner's post-attempt re-check still sees the file missing, so the cooldown row persists. For a TV show the identical no-op then repeats every 24 hours indefinitely; for a movie it fires once — the post-attempt directory hash is saved, the unchanged-hash early return skips the title on every later tick, and the no-op recurs only when the directory contents change. The title's DB row degrades to whatever can be derived without metadata, and the frontend loses the title's metadata at the next sync. This is the sharpest edge of "filesystem is ground truth + operator freeze": the freeze protects the *file*, but only if the file exists. Recovery is manual: either restore the file from backup or temporarily set `update_metadata: true` for one scan tick. The overrides variant is only partly better — the recreated file contains *only* the overridden keys, not the TMDB payload.
...
- **Detection is truthiness, not presence** (verified in `tmdbConfig.mjs`): `getOverride()` returns `config["override_" + field] || null`, so an empty-string override is identical to no override; `getMetadataOverrides()` returns `config.metadata || null`, and while a present-but-empty `{}` technically passes the check, its merge is a no-op — behaviorally indistinguishable from absence. There is currently **no way to express "opt in to override semantics for this key with an empty value"**. Decided 2026‑07‑07 (G‑2, Branch 2, **not yet implemented**): detection moves to presence-based opt-in — a present key, even `{}` or `""`, becomes an explicit signal distinct from absence. Until Branch 2 ships, current behavior is truthiness; do not document or build against presence semantics as if they exist.
...
| [Planned] pristine-base column | SQLite `main` | Yes (from a genuine TMDB fetch) | Design only — unshipped |
...
- `insertOrUpdateMovie()` carries a change-guard: the `ON CONFLICT ... DO UPDATE` only fires `WHERE movies.directory_hash IS NULL OR movies.directory_hash <> excluded.directory_hash OR movies.backdrop_focal_suggested IS NULL` — an unchanged movie is not rewritten.
...
Schema note: `tv_shows` has a `metadata TEXT` column carrying the show's metadata content; `movies` has **no equivalent column**. This gap is the root cause of the movie hash-clobber defect (F-1, below) and one of the two pristine-base hazards (G-5, below). Decided 2026-07-07 (Branch 1, merging G-5 + F-1): one migration adds both a `metadata` and a `pristine_metadata` column to `movies` — not yet implemented.
...
**Admin "refresh" is delete-only (A-3).** `POST /api/tmdb/cache/refresh` (admin) calls `refreshTmdbCacheEntry()` in `node/sqliteDatabase.mjs`, which only `DELETE`s the matching row — no refetch occurs, despite the route name and its `'Cache entry refreshed successfully'` response message. Two additional sharp edges verified in code: (a) the function computes the key with the plain `generateTmdbCacheKey()`, so it can never match a blurhash-enhanced entry's `_blurhash` custom key — those entries are unreachable from this endpoint; (b) `refreshed: false` actually means "no row existed", not "refresh failed". Decided 2026-07-07 (A-3): implement the **eager refetch** the endpoint claims to do — not yet implemented.
...
**`missing_data_media`** (show/movie-level metadata cooldown): *absent* → **marked on attempt** — the scanner marks before the retry runs, not only on failure (`movie-scanner.mjs` marks when `missingMetadata` before invoking the generator) → *cooldown-active* (24h, `RETRY_INTERVAL_HOURS`) → *cleared* only when both metadata and images resolve. A same-tick success therefore marks then clears in one pass; a row's presence does **not** mean "currently failing". Planned change (Branch 3, merging G-1a + R-1 + R-2, not yet implemented): move the mark to after the retry, distinguish frozen (`update_metadata: false`) items from genuine failures, and make the images-retry gate freeze-aware.

**`episode_metadata_missing`** (air-date-aware sparse-episode backfill): *absent* → *inserted with attempt counter* → *retried every ≥3 days* (`EPISODE_MISSING_RETRY_DAYS`) → *cleared on resolution* (`clearEpisodeRetry`) → *given up* past 90 days after air date (`EPISODE_MISSING_WINDOW_DAYS` in `node/lib/metadataGenerator.mjs`; `isEpisodeBackfillDue()` simply returns false) — **given-up rows are never pruned**. Bounded by real episode count, but permanent for genuinely unfillable episodes. Planned (Branch 3, R-3 additive): surface an `expired` flag from `refreshMissingEpisodes()` and clear on it.
...
- **Scanner immediate path (content-aware).** `movie-scanner.mjs` parses `metadata.json` and folds its content into `generateMovieHashes()`; `tv-scanner.mjs` calls `generateTVShowHashes()` with the freshly saved show (whose `tv_shows.metadata` column carries content).
- **Scheduled sweep (content-blind for movies — F-1).** A cron at minutes 5/20/35/50 (`node/app.mjs`) runs `updateAllMovieHashes(sinceTimestamp)` / `updateAllTVShowHashes(sinceTimestamp)` with `sinceTimestamp = now − 16 min`, processing titles whose media mtime moved in that window. For TV this is content-aware (`getTVShows()` rows include `metadata`). For movies it is **content-blind**: `getMovies()` rows have no `metadata` column, so `generateMovieHashes()` hashes `metadata: undefined` — and can **clobber the scanner's fresh content-aware movie hash with a content-blind one**, reopening the frozen-hash → stale-frontend bug class this table was already patched for once. Decided 2026-07-07 (Branch 1): add the `metadata` column to `movies` and feed it from the scanner so the sweep can be content-aware — not yet implemented.
- **First-scan skip (F-2).** Both scanners skip the immediate hash for brand-new titles: movies because `dirHashChanged = existingMovie && ...` is false when there is no existing row; TV because the regeneration is gated `if (storedHash)`. A new title's first hash therefore comes only from the (movie-content-blind) sweep — and only if its media mtime lands inside the 16-minute lookback — or from the read-side fallbacks below. A new title can be absent from the bulk sync endpoint for ~15 minutes or more. Planned fix (Branch 4): `dirHashChanged || !existingMovie` and `if (dirHashChanged)` respectively — not yet implemented.
...
- `getMediaTypeHashes()` (backs the bulk endpoint `GET /api/metadata-hashes/:mediaType`) — on an empty table, calls `updateAllMovieHashes(db)` / `updateAllTVShowHashes(db)`. Both functions' only parameter is `sinceTimestamp`; the truthy `db` object becomes an Invalid `Date`, every mtime comparison evaluates false, and the bootstrap **processes zero rows**. Verified current. Planned fix (Branch 4): call both with no arguments.
- The single-title route (`node/routes/metadataHashes.mjs`) additionally backfills a missing movie hash via `generateMovieHashes()` behind a plain GET — deliberately guarded (404-before-write, generate only when no row exists) and accepted as safe (A-5, decided 2026-07-07); the code comment there explains why it must *not* regenerate on every GET (it would write a content-less hash over the scanner's content-aware one).
...
### [Planned] Pristine-base column — forward-looking, unshipped

This entry describes a converged design (Branches 1–2), **none of which exists in code yet**. It is recorded here so the lifecycle is pinned before implementation. (Why it is a SQLite column rather than a file: the design note at the end of §1.5.)
...
*never-fetched* → *populated* — written **only** from a genuine TMDB fetch (`generateForMovie()` / `generateForShow()` writing `tmdbData` before `applyMetadataOverrides()`), never re-synced from `metadata.json` content → *trusted as merge base* — **iff** a `tmdb_id` is currently set in `tmdb.config` → *forced-empty* — when the id is absent or cleared (the wrong-auto-match correction case: clearing the id must also distrust the base) → *bootstrapped* — a one-time **by-id-only** fetch for existing libraries; never a by-name search.

Two hazards the implementation must account for, both verified against current code:

- **Movie/TV schema asymmetry (G-5).** `tv_shows` already has a raw-content-adjacent `metadata` column; `movies` has neither it nor anything like `pristine_metadata`. The migration must add columns to both tables (and Branch 1 folds the F-1 `metadata` column into the same migration to avoid a third ad hoc schema change).
- **Unvalidated-id trust leak from the "already up to date" branch (G-3).** `node/lib/metadataGenerator.mjs` `generateForShow()` has a branch where show metadata is fresh but a TMDB id is still needed for season processing: it runs a **name search** (`fetchComprehensiveMediaDetails` by name, first result, no disambiguation) and writes the resulting id into config via `updateTmdbConfigWithId()` — without ever validating that id against the show's actual metadata. Under "trust the base iff an id is set", this path can silently make an unvalidated auto-match trusted. The auto-match risk is therefore reachable from the *up-to-date* branch, not only the fresh-fetch branch. Planned mitigation (Branch 2): read the id from the existing `metadata.json` first and fall back to search only if absent.
...
1. **Opaque content reads for hash fingerprinting and persistence.** `movie-scanner.mjs` `scanMovies()` parses `metadata.json` into a `metadataFingerprint` that is folded whole into `generateMovieHashes` (with a `{mtimeMs, size}` stat marker as the unparseable-file fallback) so the frontend-facing hash moves on a metadata rewrite. `tv-scanner.mjs` `processShowMetadata()` similarly parse-and-re-serializes the file into an opaque `metadata` string for the DB column and show hash. In both cases the parse only normalizes bytes — no individual field is ever inspected or branched on.
...
### 4.2 Scanner ⇄ freeze switch: freeze-blind gates, freeze-aware callees, fail-closed

**Contract.** The scanner's own gate computation (`missingMetadata`, image staleness, 24h cooldown) stays **freeze-blind**: it never consults `update_metadata`. Enforcement of the freeze belongs to the generator — `metadataGenerator.mjs` `generateForShow()` / `generateForMovie()` both check `isUpdateAllowed(tmdbConfig)` and no-op TMDB writes when frozen. **However**, anything the scanner calls that can *independently* trigger a TMDB write must have freeze-awareness threaded in explicitly, and it must **fail closed** (no argument ⇒ no write).
...
**Known gap (R-2, Branch 3 — decided 2026-07-07, not yet implemented).** Because the scanner's images gate is currently freeze-blind, a frozen TV show with a permanently missing image is re-offered to the generator every scan tick (the TV scanner computes its gates before the unchanged-hash fast-skip); the generator's freeze check no-ops each invocation, but the wasted round trip recurs unboundedly. A frozen movie in the same state is re-offered only when its directory contents change, because the movie scanner's unchanged-hash early return runs before its gates (see §3). This is not an accepted permanent cost: Branch 3's decided fix gates the images-missing retry on `isUpdateAllowed()` (leaving the metadata-missing branch ungated), making the scanner's images-retry gate freeze-aware. When Branch 3 ships, the freeze-blind contract above narrows accordingly — the scanner's gate computation will consult the freeze flag for exactly that one retry decision, and this section must be updated to state the narrowed contract.
...
- **Decided 2026-07-07 (A-1) — planned fix, NOT accepted:** `GET /rescan/tmdb` (`app.mjs`) currently has **no authentication** while triggering the single most privileged mutation in the surface — a full-library forceRefresh (`runDownloadTmdbImages({ fullScan: true })`), which wipes managed images and re-pulls everything. Unlike V-5, this is *not* an accepted boundary: Branch 9 adds `authenticateWebhookOrUser` (already imported in `app.mjs`; its sibling scan route `POST /media/scan` already uses it). Not yet implemented — until it ships, treat this route as exposed and rely on network-level controls.
...
| GET | `/rescan/tmdb` | **none** | Full-library TMDB force-refresh: runs `runDownloadTmdbImages({ fullScan: true })`, which wipes and re-downloads managed images library-wide. | The single most privileged mutation in the surface, currently unauthenticated. Decided 2026-07-07 (A-1): add `authenticateWebhookOrUser` — not yet implemented. **Not** in the decided S-2 task-manager gating scope (that covers only the write-heavy `/api/admin/metadata/*` routes); if S-2 gating is ever extended here, it must wrap — not replace or revert — the A-1 auth middleware. |
...
| POST | `/api/tmdb/cache/refresh` | admin | "Refresh" a cache entry for `{endpoint, params}`. | Currently delete-only: `refreshTmdbCacheEntry` removes the row but performs no refetch, despite the success message. Decided 2026-07-07 (A-3): implement the eager refetch — not yet implemented. |
...
- **Freeze** — `update_metadata: false` in a title's `tmdb.config`. A reversible halt to all TMDB fetches and `metadata.json` writes for that title, enforced one layer below the scanner in `MetadataGenerator` (the scanner itself stays freeze-blind for its own gate computation). Overrides still apply while frozen (`reason: 'updates-disabled-overrides-applied'`). Not a "no TMDB match" marker — the scanner's cooldown table currently cannot distinguish frozen / no-match / transient-failure (Branch 3 addresses this; not yet implemented).

- **Two-gate retry model** — the scanner's TMDB-retry decision in `node/components/media-scanner/domain/movie-scanner.mjs` / `tv-scanner.mjs`: the **metadata gate** (missing `metadata.json` → retry only after the 24 h cooldown in `missing_data_media`, because a lookup failure isn't worth retrying every 3-minute tick) and the **images gate** (missing image files → retry with no cooldown, because image failures are typically transient). A directory-hash change forces a retry regardless of either gate. The gates run at their stated cadence only in the TV scanner, which computes them every tick before its unchanged-hash fast-skip; the movie scanner early-returns on an unchanged directory hash *before* either gate is evaluated, so for movies both gates fire only when the directory contents change. Known gap: the images gate has no freeze-awareness, so a frozen TV show with a permanently missing image retries every tick forever (Branch 3, decided 2026-07-07, not yet implemented).

- **Pristine base** — *planned, not yet implemented*: a SQLite column holding the raw, pre-override TMDB response for a title, populated only by a genuine fetch (never re-synced from `metadata.json`), trusted as the merge base for override application iff `tmdb_id` is currently set, and forced empty when the id is cleared (the wrong-auto-match correction path). Lives in SQLite rather than a file because it is derived state — a cache of TMDB's response — not ground truth. Requires a movies-table schema addition (Branch 1) since only `tv_shows` has a raw-content column today.

- **Content-aware vs. content-blind hash** — a metadata hash is *content-aware* when the parsed `metadata.json` content is folded into its input (the scanner's immediate path: `movie-scanner.mjs` passes a metadata fingerprint into `generateMovieHashes()`), so TMDB-content edits move the hash and propagate to the frontend. It is *content-blind* when computed from a DB row that carries no metadata content — the scheduled `updateAllMovieHashes()` sweep is content-blind for movies because the `movies` table has no `metadata` column, and can clobber a fresh content-aware hash (Branch 1 / F-1, decided 2026-07-07, not yet implemented). TV show hashes are content-aware in both paths via `tv_shows.metadata`.
