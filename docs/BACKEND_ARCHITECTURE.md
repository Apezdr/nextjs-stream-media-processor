...
**Terminology.** One term per concept, used consistently: **freeze** (the `update_metadata: false` switch — see Appendix C), **pristine base** (the raw pre-override TMDB snapshot — column and population shipped with Branch 1; its trust semantics remain planned), **ground truth**, and **derived state**.
...
| `node/integrations/discord/MEDIA_ADMIN_COMMANDS_PLAN.md` | corrected (D-2, this branch) — now honestly labeled PROPOSAL/BACKLOG | Historically self-reported its 17-item checklist as 100% complete when only the `/tasks` command (plus the `getAdminByDiscordId()` helper) had shipped. The D-2 correction adds a status banner, marks each checklist item against git reality (1 ✅, 1 🟡, 15 ❌), and forbids citing the doc as shipped behavior. | None — keep as the design reference if Discord media administration is revisited. |
...
2. **Self-healing config that silently reverts to defaults if lost.** `app_config.settings` holds named settings documents. Both `checkAutoSync()` in `node/database.mjs` and the autoCaptions config reader in `node/components/caption-generator/data-access/caption-config.mjs` *insert the default document if none exists*. Convenient for first boot, dangerous afterward: if these documents are ever lost, the system does not fail — it quietly resumes with defaults, discarding whatever the operator had configured. **Implemented (M-4, this branch):** the durability posture is documented in README ("MongoDB durability & backups") and `.env.example` (MongoDB block) — `Users` and `app_config` are named as the collections the operator-confirmed external backup/snapshot policy must cover — and both seed sites emit a warn-level revert-detection audit line (grep marker `app_config.settings audit`): normal on a genuinely fresh install, a loss/revert signature on an established deployment. Deliberate setting *changes* are written by the frontend, so this backend can only witness the loss/reseed half; a full change-audit would live frontend-side.
...
**Owns:** All interpretation of per-title `tmdb.config` files. `node/utils/tmdbConfig.mjs` is the only legal reader/writer of that file's schema: load/save (`loadTmdbConfig`, `saveTmdbConfig`), validation and defaults (`validateTmdbConfig`), the freeze predicate (`isUpdateAllowed` — `update_metadata !== false`, defaulting to allowed), override accessors (`getMetadataOverrides`, `applyMetadataOverrides`, and the presence-based opt-in check `hasMetadataOverrideKey` — the single source of truth for "is this title override-managed", G-2), the id ratchet and its provenance rule (`updateTmdbConfigWithId` for the name search — add-only, stamps `tmdb_id_source: auto`; `pinTmdbIdentity` / `resolveIdentityPin` for identity providers — a provider may replace an `auto` or provider-owned pin, never a `manual` one; `getIdentityProvenance` for the "id with no source reads as manual" rule; `stampProvenanceForOperatorWrite` for the admin PUT, which marks an added or changed id `manual`), and path resolution (`getTmdbConfigFilePath`). Every other subsystem must go through these exports rather than hand-parsing the JSON. The precedence table lives on `resolveIdentityPin`'s doc comment and in `node/integrations/identity/README.md`; it is pinned row by row in `tests/unit/utils/tmdbConfig-identity.test.mjs`.
...
**Owns:** The single request/cache/retry chokepoint for TMDB HTTP traffic in `node/utils/tmdb.mjs`: response caching (default TTL 1440 hours / 60 days, backed by the `tmdb_cache.db` SQLite file), retry/backoff logic (429 rate-limits plus transient network errors and 5xx responses — T-5, Branch 7), a process-wide request-concurrency ceiling every caller draws from (`TMDB_REQUEST_CONCURRENCY`, default 8 — T-4b, with a per-collection fan-out cap layered under it, T-4a), ETag-based conditional revalidation (T-1), by-id vs. by-name lookup semantics, and response aggregation helpers (collections, cast, trailers). `forceRefresh` bypasses the response cache read. The whole pipeline is native Node; the former Python TMDB scripts are removed.

**Does not own:** Deciding when to fetch (generator) or what the results mean for disk state (generator/downloader).

**Accepted posture — no circuit breaker (T-7, decided 2026-07-07).** There is deliberately no circuit breaker for a sustained TMDB outage. Protection is layered instead: per-request retry/backoff in this boundary, plus the per-title 24-hour `missing_data_media` cooldown at the scanner — so each in-flight title fails once and goes quiet rather than hammering a down API. This is accepted on the assumption that TMDB outages stay rare and short; revisit only if an observed incident leaves scan duration unacceptable now that the T-4/T-5 client-hardening fixes have landed (Branch 7).
...
**Owns:** Image filename conventions on disk, existence-check-gated downloads, blurhash sidecar generation, and backdrop focal-point detection. The downloader (`node/utils/imageDownloader.mjs`: `downloadImage`, `downloadImageWithBlurhash`, `downloadMediaImages`, `downloadSeasonPoster`, `downloadEpisodeThumbnail`) is deliberately a dumb existence-check writer on the top-level poster/backdrop/logo path (`downloadImage` / `downloadMediaImages`): there it has no staleness logic of its own, and the generator alone decides when to delete a file so the downloader is forced to re-fetch. The two grandfathered exceptions are `downloadSeasonPoster()` and `downloadEpisodeThumbnail()`, which carry their own age-based TTL checks inside the downloader (§4.3). There are three independent blurhash entry points: (1) the scheduled sidecar scan pipeline (`node/utils/blurhashNative.mjs` with the worker pool in `node/lib/blurhash-pool.mjs`, persisted via `node/sqlite/blurhashHashes.mjs`); (2) blurhash generated at image-download time (`generateImageBlurhash` in `node/utils/imageDownloader.mjs`); (3) blurhash embedded into cached TMDB API responses (`node/utils/tmdbBlurhash.mjs`, cached via `node/sqlite/tmdbBlurhashCache.mjs`). **Implemented (B-2b, this branch):** all three draw their encode sizes from the single policy module `node/utils/blurhashSizePolicy.mjs` — a zero-behavior-change unification (same effective sizes as before, now single-sourced). The two sidecar writers share one table (via `sidecarBlurhashSizeForImageType()` / `sidecarBlurhashSizeForFilename()`: backdrops/thumbnails `small`, posters `medium`, logos `large`), and the TMDB-proxy pipeline's per-context sizes are the named `TMDB_PROXY_BLURHASH_SIZES` constants, with its **details/collection posters at `large` documented as a deliberate divergence** from the sidecar `medium` (browse/search preview art vs. library storage) rather than drift. Changing any value in that module is a policy change with invalidation costs (sidecar version-bump — which today covers movies only, not TV — and size-keyed tmdb blurhash cache rows); the module doc and its freeze-tests enforce that. The `.blurhash` sidecar lifecycle these pipelines feed is cataloged in §3. Backdrop focal detection lives in `node/utils/backdropFocalDetector.mjs` (`detectBackdropFocal`, face-api based).
...
**Owns:** Cron cadence and concurrency guarantees — **only for the paths that go through the task manager.** `node/lib/taskManager.mjs` defines the `TaskType` priority enum, per-type `concurrencyLimits` (e.g. one scan, one blurhash, one cache cleanup at a time), and `exclusiveGroups` that keep media scans, metadata hashing, and blurhash work mutually exclusive — the sole exclusivity group since S-4 removed the `[DOWNLOAD, CACHE_CLEANUP]` pairing (poster-collage generation, the only `DOWNLOAD` caller, shares no files or tables with the cache sweeps it was serializing). Jobs are scheduled with `node-schedule` in `node/app.mjs` and wrapped in `enqueueTask`. The scheduled blurhash job enqueues under `TaskType.BLURHASH`, which exists in the enum and carries a concurrency limit of 1 (shipped fix, S-1).
...
(A second historical limit was closed by V-1 in Branch 11: the frames/spritesheet/clips cleanups used to be scheduled *twice* in `node/app.mjs` — once through the `CACHE_CLEANUP` gate at module level and again inside `scheduleTasks()` calling the clear functions directly, outside all concurrency and exclusivity guarantees. The ungated duplicates are deleted; every cleanup now runs exactly once, through the gate, and a comment in `scheduleTasks()` warns against re-adding a direct one.)
...
Five cache directories are declared in `node/utils/utils.mjs`: `general`, `spritesheet`, `frames`, `video_clips`, and `video_transcode`. Scheduled cleanup covers all of them — general cache, video clips, spritesheets, frames, original segments (`-original.mp4` files inside the video-clips directory on a short 8-minute access-time window), and, since V-2 (Branch 11), video transcodes (`clearVideoTranscodeCache` in `node/utils/utils.mjs`: 30-day mtime window, 4:15 AM daily through the `CACHE_CLEANUP` gate — previously the only cache with no eviction at all, and the mechanism that reclaims entries stranded under pre-V-3 cache keys). Every cleanup is scheduled exactly once, at module level, through the task-manager gate (V-1 — see 2.9). Since Branch 11 the full-transcode cache key also carries the source file's `info.uuid` identity component (V-3 — the video-clips key already did), and `getInfo()` in `node/infoManager.mjs` regenerates a `.info` sidecar whose recorded source size/mtime no longer matches the file on disk (V-4) — so replacing a video in place rotates the uuid and both uuid-keyed caches go stale correctly.
...
This is real, working functionality today — distinct from `node/integrations/discord/MEDIA_ADMIN_COMMANDS_PLAN.md`, a mostly-unbuilt design proposal (roughly one of its seventeen items shipped; since the D-2 correction the doc says so itself, with a status banner and an honest per-item checklist). Do not read the plan document as a description of current behavior.
...
### 2.13 Identity providers (Radarr / Sonarr) — shipped 2026-09-21

**Owns:** Asking external media managers for the exact TMDB id of every folder they manage, and reconciling those ids into `tmdb.config` so the name search never runs for managed content. `node/integrations/identity/` holds the provider contract (`provider.mjs` — `IdentityProvider`, claims keyed by library-relative path `movies/<folder>` / `tv/<folder>`), the registry (`registry.mjs` — `PROVIDER_CLASSES`, order = precedence; enabled purely by each provider's env), the per-tick index (`index-builder.mjs` — every provider pulled once in parallel, a failing provider contributes nothing, cross-provider disagreements kept), the reconciler (`reconciler.mjs` — walks the index, calls `pinTmdbIdentity` per folder, produces the report: written / conflicts / provider-only / unmanaged), the HTTP surface (`routes.mjs`) and the service app.mjs uses (`index.mjs` — `reconcileForTick`, `handleWebhook`, `getStatus`). The two shipped providers share an `*arr` base (`arr/arrProvider.mjs`: v3 API, `X-Api-Key`, optional root map, Webhook payload shape) with thin `radarr.mjs` / `sonarr.mjs` subclasses.

**When it runs (decoupled from the scan tick, 2026-09-22).** The service has its own job (`IDENTITY_RECONCILE_INTERVAL_SECONDS`, default 60, `0` = off, started from `app.listen` and stopped in `gracefulShutdown`) with reason `identity-tick`, plus forced runs at the top of `runGenerateList` (`scan-tick`), from `POST /api/identity/reconcile` (`manual`), and after any provider webhook that names a folder (`<provider>-webhook`). Unforced runs go through a change detector: each provider list is fingerprinted over `(path, tmdbId, hasFile)` in `index-builder.mjs` and the library's folder names in `reconciler.mjs`; if neither moved since the last real run the report keeps its `at` and gets a fresh `checkedAt` with `unchanged: true`, and nothing on disk is read or written. One provider failing on an unforced run leaves the last good report untouched (its failure is on `lastFetch.error`); a media type with no successful provider is reported as `covered: false` and never as unmanaged. A repair found outside the scan tick requests an early scan. The report publishes `checkedAt`, `checkedReason`, `unchanged`, and `staleAfterMs` (3 × the job interval) so a page judges freshness on the processor's own cadence. Provider-only rows also carry the provider's own availability verdict — `released` (Radarr `isAvailable`; Sonarr `status !== 'upcoming'`), `arrStatus` (the app's lifecycle word, verbatim) and `monitored` — all nullable, never guessed, so the admin page can distinguish "not released yet" from "released, not downloaded" from "not monitored"; `released` is folded into the change-detector fingerprint (it flips once, on release day), `monitored` and `arrStatus` deliberately are not. The same rows carry `art` (`{ poster, backdrop }`: the app's remote poster/fanart URLs from `images[].remoteUrl`, never its local `/MediaCover` paths; null when absent; not in the fingerprint), so the admin page can show artwork for titles that are not in the library.

**Claims without a TMDB id (2026-09-22).** A manager can know a folder without knowing its TMDB entity (Sonarr's TVDB→TMDB mapping is missing for some new shows). The *arr base emits such an item as an *unidentified* claim (`tmdbId: null`, `externalIds` non-empty); the index builder offers it to `integrations/identity/external-id-resolver.mjs`, which calls `findTmdbIdByExternalId()` in `node/utils/tmdb.mjs` (TMDB `/find`, IMDb first then TVDB for tv, 24-hour cache) and promotes the claim on a hit (`resolvedVia`). What still cannot be resolved is kept in `IdentityIndex.unidentifiedByPath` and reported under `managedUnidentified` with the folder's own pin (`localPin`, read through the config module), never under `unmanaged`; the precedence rule is never offered a null id. The TMDB client is imported lazily by the resolver so the identity package stays light for callers and tests that never see such a claim. Motivation: on 2026-09-22 a Radarr fix during a ten-minute scan tick stayed invisible for nine minutes because the tick was the only thing that rebuilt the report.

**How a repair propagates without new scanner branches.** The reconciler also runs at the top of `runGenerateList`, before the scanners read anything. A `write` (id added or replaced) saves `tmdb.config` normally, so its mtime moves past `metadata.json`'s — exactly what the scanner already reads as an operator edit (`staleByConfig` → `fullScan: true` → generator force-refresh by the pinned id, old art wiped, new art pulled, hashes move, frontend resyncs). A `stamp` (provider agrees with an existing `auto` pin; only provenance changes) saves with `preserveMtime`, so bookkeeping never triggers that repull. `keep` and `conflict` write nothing.

**Webhook (push).** `POST /api/identity/webhook/:provider` accepts the provider's Webhook connection. The payload carries the same ids as the list endpoint, so the one folder it names is reconciled immediately and, for import/upgrade/rename/add events, the normal scan tick is started early (coalesced in app.mjs `requestEarlyScan`). Auth is the shared webhook id; because Radarr/Sonarr offer Basic credentials but no custom headers, the Basic password is lifted into `X-Webhook-ID` (`basicAuthToWebhookHeader`).

**Does not own:** The precedence rule (that is the `tmdb.config` module, §2.3/§4.6), episode numbering (still filename-derived), deletion of anything, or any frontend change (the sync already carries whatever id the processor decides).

**Failure posture:** a provider that is down yields no claims for that tick and a warning; if every provider fails the library is left untouched (`skipped: no-provider-data`). The reconciler never throws into the scan.

**Config:** `RADARR_URL` + `RADARR_API_KEY`, `SONARR_URL` + `SONARR_API_KEY`; optional `<PREFIX>_ROOT_MAP`, `<PREFIX>_TIMEOUT_MS`; `IDENTITY_UNSOURCED_PINS=manual|auto` (default `manual`: pins written before provenance existed are never overwritten, only reported). All unset = today's behaviour byte for byte.

**Key files:**
- `node/integrations/identity/README.md` — layout, precedence table, config, adding a provider
- `node/integrations/identity/provider.mjs`, `registry.mjs`, `index-builder.mjs`, `reconciler.mjs`, `routes.mjs`, `index.mjs`
- `node/integrations/identity/arr/arrProvider.mjs`, `radarr.mjs`, `sonarr.mjs`
- `node/utils/tmdbConfig.mjs` — `resolveIdentityPin`, `pinTmdbIdentity`, `getIdentityProvenance`, `stampProvenanceForOperatorWrite`
- `node/app.mjs` — `identityService`, `requestEarlyScan`, the `reconcileForTick()` call in `runGenerateList`
...
```
Absent ──► Downloaded/managed (DB-tracked *FilePath) ──► Orphaned │ Stale-by-config │ Stale-by-source-URL ──► deleted (enforce) ──► re-downloaded
...
- **Stale-by-source-URL (I-3, implemented this branch).** The DB-tracked file matches the current expected path, but its recorded download provenance (`*_source_url` column) differs from the current effective URL — a genuine upstream TMDB art change (new CDN path, same extension) that both mtime-based checks are structurally blind to. Reconcile deletes it so the downloader re-fetches from the new URL; the re-download then records the new provenance, so the check self-converges. **Hard invariant:** a NULL stored provenance means "unknown — row predates I-3" and *skips* the comparison (it converges by adoption instead); treating NULL as a mismatch would judge the whole library stale on the first post-deploy pass. Same strict `previousPath === expectedPath` ownership precondition as the stale-by-config branch, and the same `SCANNER_RECONCILE_MODE` gate (dry-run by default until I-1 ships).
...
The `.blurhash` sidecars are also the *only* blurhash source the DB/frontend actually consumes. The fetch pipeline *can* embed TMDB-blurhash fields into its responses, but nothing scanner-side ever read them back, so since B-1 (Branch 10) the scanner-triggered `fetchComprehensiveMediaDetails()` calls in `node/lib/metadataGenerator.mjs` pass a hard `includeBlurhash: false` — regardless of the generator's `generateBlurhash` config flag, which continues to govern only the sidecar downloads. This also keeps scanner fetches on the plain TMDB cache key (the one the A-3 admin refresh endpoint can address). The embedded enhancement remains available to the interactive `/api/tmdb/*` proxy routes, which request it explicitly via their `blurhash=true` query parameter.
...
| Pristine-base column (`pristine_metadata`) | SQLite `main` | Yes (from a genuine TMDB fetch) | Column + population shipped (Branch 1); trust/merge semantics decided — not yet implemented |
...
Schema note: `tv_shows` has a `metadata TEXT` column carrying the show's metadata content; historically `movies` had no equivalent, which was the root cause of the movie hash-clobber defect (F-1, below) and one of the two pristine-base hazards (G-5, below). **Implemented (Branch 1):** one startup migration (`migrateToSchemaParityColumns()` in `node/sqliteDatabase.mjs`, ALTER TABLE ADD COLUMN only, idempotent) adds `movies.metadata` (F-1) plus `pristine_metadata` on both tables (G-5) plus the three I-3 `*_source_url` columns on both tables. **Implemented (I-3, this branch):** the `*_source_url` columns are populated and consumed — they record per-kind image download *provenance* (the URL the on-disk managed file was actually fetched from), carried opaquely from the generator's return (`sourceUrls`, like `pristineMetadata`) and COALESCE-preserved per column on both upserts; a `null` per-kind update means "no new information". Population happens on a real download (true provenance) or, once, by NULL-bootstrap adoption on a cache-hit (a pre-I-3 row adopts the current effective URL as assumed provenance — "unknown" converges by adoption, never by deletion). A stored non-NULL URL is deliberately never overwritten without a download, so a pending stale-by-URL mismatch survives dry-run reconcile passes. The movie upsert's change-guard carries three matching bootstrap escape hatches (`*_source_url IS NULL AND excluded IS NOT NULL`) so adoption lands on an unchanged directory and converged rows stop rewriting.

---

### TMDB cache row — the most defect-dense entity in the system

Table `tmdb_cache` (its own DB file, `tmdb_cache.db`; schema in `node/sqliteDatabase.mjs`): `cache_key` (UNIQUE), `endpoint`, `request_params`, `response_data`, `created_at`, `expires_at`, `last_accessed`, `etag` (added by T-1, Branch 7 — an idempotent ALTER converges pre-existing databases).
...
**Conditional revalidation (T-1, implemented in Branch 7).** The formerly dead ETag plumbing is live end-to-end: `setTmdbCache()` takes the captured ETag as its 6th parameter and persists it into the `etag` column, and `makeTmdbRequest()` itself is the real caller — when a refetch is about to happen (row expired, or `forceRefresh` skipped the read) and no explicit `ifNoneMatch` was supplied, it looks the stored row up via `getTmdbCacheEntryAnyAge()` (an expiry-blind read added for exactly this) and sends its `etag` as `If-None-Match`. A 304 response then re-ups the stored row — same payload, same ETag, fresh TTL window — and returns it flagged `_notModified: true`, so freshness extends past the 60-day TTL without payload transfer. A 304 with no stored row (possible only with a caller-supplied `ifNoneMatch`) still falls through to the retry/error path.

**Cache-write telemetry (T-6, implemented in Branch 7).** `setTmdbCache()` returns an explicit `true`/`false` (success vs. invalid-TTL skip or caught error), and `withApiCacheSpan()` (`node/lib/apiTracer.mjs`) branches on `operation: 'SET'`: writes record `api.cache_write_success` instead of `api.cache_hit`, so read-side hit-ratio dashboards are no longer polluted by writes. For void writers that still flow through a SET span (e.g. the blurhash cache), only an explicit `false` counts as failure — they signal failure by throwing, which the span's error counter records.
...
3. **Movie scan-time pregeneration — fully working (C-1, implemented this branch).** `movie-scanner.mjs` `generateChapterFileIfNotExists()` now mirrors the TV helper: it probes `chapterInfo()`, and when the mp4 carries chapter atoms it creates the `chapters/` directory and writes the VTT file. (Historically this was a silent no-op — the call passed the `quietMode` boolean into `generateChapters`'s `chapterData` parameter, which short-circuited the probe and discarded the returned VTT while still burning a `getVideoDuration()` ffprobe per reprocess.) Movies without chapter atoms write nothing and keep relying on the lazy path (1). Two properties worth preserving: the write runs (via `processChapters()`) **before** `final_hash` is computed in `scanMovies()`, so the stored directory hash already includes the new VTT and pregeneration converges in one pass instead of re-triggering `dirHashChanged`; and pregeneration is **incremental only** — it fires on the reprocess path (new titles, changed directories), so already-stable movies keep using the lazy route until something else reprocesses them.
...
- `getMediaTypeHashes()` (backs the bulk endpoint `GET /api/metadata-hashes/:mediaType`) — on an empty table, runs a full-library `updateAllMovieHashes()` / `updateAllTVShowHashes()` bootstrap. **Fixed (with Branch 1):** it previously passed the `db` handle into the functions' `sinceTimestamp` parameter, so the truthy object became an Invalid `Date`, every mtime comparison evaluated false, and the bootstrap silently processed zero rows.
...
### Pristine-base column — schema + population shipped (Branch 1); trust semantics decided, not yet implemented

This entry describes the converged pristine-base design. **Branch 1 is merged**: the `pristine_metadata` column exists on both tables and is faithfully populated from genuine fetches. Branch 2 as merged shipped the override *helpers* (G-2 presence-based opt-in, G-3 trusted-id adoption) only — the *trust/merge* lifecycle stages below (trusted-as-merge-base, forced-empty, by-id bootstrap) are **decided but not yet implemented**, with no branch currently scheduled. (Why it is a SQLite column rather than a file: the design note at the end of §1.5.)
...
*never-fetched* → *populated* — **shipped**: written **only** from a genuine TMDB fetch this invocation (`generateForMovie()` / `generateForShow()` return `pristineMetadata` — `tmdbData` serialized before `applyMetadataOverrides()` — and the scanners persist it opaquely through `saveMovie()`/`saveTVShow()`; a save without a fresh payload COALESCE-preserves the stored value), never re-synced from `metadata.json` content → *trusted as merge base* — **planned**: **iff** a `tmdb_id` is currently set in `tmdb.config` → *forced-empty* — **planned**: when the id is absent or cleared (the wrong-auto-match correction case: clearing the id must also distrust the base) → *bootstrapped* — **planned**: a one-time **by-id-only** fetch for existing libraries; never a by-name search.
...
- **Unvalidated-id trust leak from the "already up to date" branch (G-3) — resolved by Branch 2 (merged).** `node/lib/metadataGenerator.mjs` `generateForShow()` has a branch where show metadata is fresh but a TMDB id is still needed for season processing. It now adopts the trusted id from the fresh `metadata.json` (written by a previous full generation) and persists it via the add-only ratchet; the unvalidated **name search** (first result, no disambiguation) runs only as a last resort when the file carries no usable id — e.g. an override-only file recreated while frozen. The residual search fallback's auto-match risk is reduced by T-3's year heuristic (implemented this branch): `pickSearchResultByYear()` in `node/utils/tmdb.mjs` scores name-search results by the `(YYYY)` in the directory name — exact year first, then ±1 (regional/December-January straddle), then the popularity-sorted first result as before — always inside the `if (!id)` block, so the add-only id ratchet still confines it to never-matched titles, and it never manufactures a match for an empty result set.
...
**Contract.** Only `node/utils/tmdbConfig.mjs` parses, validates, or interprets `tmdb.config`. Every consumer goes through its exported accessors — `loadTmdbConfig()`, `saveTmdbConfig()`, `updateTmdbConfigWithId()`, `pinTmdbIdentity()`, `resolveIdentityPin()`, `getIdentityProvenance()`, `stampProvenanceForOperatorWrite()`, `isUpdateAllowed()`, `getOverride()`, `applyMetadataOverrides()`, `getTmdbConfigFilePath()`. This holds in all live code paths today: `tv-scanner.mjs`, `metadataGenerator.mjs`, `routes/admin.mjs`, and `integrations/identity/reconciler.mjs` all import from the module rather than hand-parsing JSON. In particular the identity providers (§2.13) never see the file: they emit claims, and the precedence decision is this module's.
...
**Contract.** Caption generation concurrency is owned solely by the hardcoded limit in `node/lib/taskManager.mjs`: `concurrencyLimits[TaskType.CAPTION_GENERATE] = 1`.

**Implemented (C-2, this branch):** the formerly dead `maxConcurrent` field is **removed** from the caption-config `DEFAULTS` in `node/components/caption-generator/data-access/caption-config.mjs` (nothing anywhere read it — it never fed the task manager despite appearing to be the tunable for exactly that), and `plans/AUTO_GENERATED_CAPTIONS.md` carries a correction where it claimed the wiring existed. A deployed Mongo `autoCaptions` doc that still contains the old field passes through `getAutoCaptionsConfig()`'s spread-merge harmlessly; nothing may start reading it. If per-deployment tuning is ever wanted, it must be built through a task-manager setter, never a second reader of a config field.
...
- **Decided 2026-07-07 (A-1) — implemented (Branch 9):** `GET /rescan/tmdb` (`app.mjs`) triggers the single most privileged mutation in the surface — a full-library forceRefresh (`runDownloadTmdbImages({ fullScan: true })`), which wipes managed images and re-pulls everything — and historically had **no authentication**. Unlike V-5, that was *not* an accepted boundary: the route now requires `authenticateWebhookOrUser`, the same level as its sibling scan route `POST /media/scan`.
...
| GET | `/api/tmdb/status` | admin | Global TMDB integration status (API-key presence, cache stats, limits). | **Renamed from `/api/tmdb/config` (A-4a, implemented this branch)** to end the name collision with the per-title editor at `/api/admin/metadata/config` — the read-only status endpoint took the rename because breaking the editor's PUT would silently block config authoring. No alias: external callers of the old path get a 404 (coordinate with the frontend as a release note; no in-repo caller existed). The response's `requests_per_minute` now reports the real 800/min limiter (was hardcoded 100). |
...
| GET | `/api/admin/metadata/config` | admin | Read one title's `tmdb.config` (validated, with defaults applied). | Per-media resource; the historical name collision with the global status endpoint was resolved by renaming that side to `/api/tmdb/status` (A-4a). |
...
### A.7 Identity providers (`node/integrations/identity/routes.mjs`, mounted at `/api` from `node/app.mjs`)

| Method | Path | Auth | Purpose | Notes |
|---|---|---|---|---|
| POST | `/api/identity/webhook/:provider` | webhook id (header **or** Basic password) or admin | A provider's Webhook connection (Radarr/Sonarr `Download`, `Rename`, `MovieAdded`/`SeriesAdd`, …). Reconciles the named folder, requests an early scan for import-like events. | 404 for an unknown/unconfigured provider, 400 for an unrecognized payload. Never deletes. |
| GET | `/api/identity/status` | webhook id or admin | Providers (redacted), last index summary, config errors, recent webhook events. | |
| GET | `/api/identity/report` | webhook id or admin | Last reconcile report: written, conflicts (manual vs provider), provider-only, managed-unidentified (provider knows the folder, nobody knows the TMDB id; carries `externalIds` and `localPin`), unmanaged, provider disagreements, errors (each list capped at 200), plus `checkedAt` / `checkedReason` / `unchanged` / `staleAfterMs`. | 202 until the first reconcile has run; `{ enabled: false }` when no provider is configured. `reason` ∈ `scan-tick`, `manual`, `identity-tick`, `<provider>-webhook`. |
| POST | `/api/identity/reconcile` | webhook id or admin | Run a full reconcile now (forced past the change detector) and return the report. | Shares one run with a concurrent tick or job pass. |
...
| `tmdb_id` | positive integer | *(absent)* | **The trust anchor**: pins which TMDB entity this directory is. Set by an operator, written once by the pipeline after a name-search match (`updateTmdbConfigWithId()` only ever *adds* an id — a search never overwrites any pin), or written/replaced by an identity provider (`pinTmdbIdentity()`, governed by `tmdb_id_source` below). Changing it marks the title stale-by-config and forces a force-refresh repull (managed images wiped, then re-downloaded) — which is also how a provider repair propagates. Invalid values (non-integer, ≤ 0) are deleted by the validator. In the pristine-base design (the `pristine_metadata` column and its population shipped with Branch 1; trust semantics decided, not yet implemented), the pristine-base snapshot is trusted as a merge base *iff* an id is currently set. |
| `tmdb_id_source` | token: `manual` \| `auto` \| a provider name (`radarr`, `sonarr`) | *(absent)* | **Who pinned `tmdb_id`** (§2.13). `manual` — a human; nothing automated may change the id. `auto` — the name search; any identity provider may replace it (the wrong-match repair). A provider name — that provider owns it and may change it; another provider may too. **Absent with an id present reads as `manual`** (`getIdentityProvenance()`), so every pin written before this field existed is protected by default; `IDENTITY_UNSOURCED_PINS=auto` widens that reading for a deliberate one-time repair pass. The admin PUT marks an id it added or changed `manual` (`stampProvenanceForOperatorWrite()`), so a hand correction is never undone. Dropped by the validator when there is no `tmdb_id` or the value is not a plain token. A provenance-only change (`stamp`) is written with the mtime preserved so it never triggers a repull. |
...
| `override_poster`, `override_backdrop`, `override_logo` | string | *(absent)* | **Per-image override.** Highest tier of the effective image-URL precedence: `override_<kind>` > `metadata.<kind>_path` > TMDB response value. Since Branch 6 (I-5), both consumers — `node/utils/imageDownloader.mjs` `downloadMediaImages()` and `node/lib/metadataGenerator.mjs` `_reconcileImageOwnership()` — resolve through the one shared `resolveEffectiveImageUrl()` in `node/components/media-scanner/domain/image-conventions.mjs`, so the reconcile step's predicted destination and the downloader's written destination cannot diverge. The value may be a bare TMDB CDN path (`/abc.jpg`, prefixed with the CDN base) **or** a full URL, which passes through verbatim (I-7a — the downloader previously prefixed unconditionally, mangling full external URLs). Override-sourced art survives a force-refresh wipe because it is re-fetched immediately after. The `getOverride()` accessor in `node/utils/tmdbConfig.mjs` is the resolver's override lookup (it gained its first caller with this change). |
| `backdrop_focal` | `'left'` \| `'right'` \| `'center'` \| `'center-left'` \| `'center-right'` \| `null` | `null` | **Manual backdrop focal point.** Mirrored by the scanners into the `backdrop_focal` DB column and exposed to the frontend as `backdropFocal`. The validator accepts every value the auto-detector (`node/utils/backdropFocalDetector.mjs` `detectBackdropFocal()`) can produce — the `center-left`/`center-right` variants were added by I-6a (Branch 6); unknown values still reset to `null` with a warning. |
