...
**Per-item isolation — "failed ≠ absent" (SI-P0, `epic/scan-integrity`).** Both scanners isolate each title: a directory that throws while being processed (a dangling link inside the folder making `calculateDirectoryHash()` stat-fail, a folder deleted between the root listing and its own `readdir`, a DB hiccup) is logged at error with its code (stack attached only for non-fs errors), counted into one per-pass summary, and **retained — never read as departed**. "Retained" means not removed: a throw before the save leaves the row untouched; a throw after `saveMovie`/`saveTVShow` leaves the row as saved, and the next tick fast-skips on the stored `directory_hash` (any stale hash row is SI-P3's reconcile to close). This holds because the title's name leaves the removal set *before* any I/O (`existingMovieNames.delete(dirName)` / `existingShowNames.delete(showName)` sit immediately after the `isDirectory()` check, ahead of the per-title `try`); a swallowed error therefore cannot leave a live title in the removal set. A folder that is genuinely gone is removed on the *next* tick, when the root listing no longer carries it. The summary is a WARN for a partial failure (`scanner: N of M movie directories failed this pass and were retained`, M = directories scanned) and an ERROR with a stable prefix when *every* directory failed (`scanner: every movie directory failed this pass … — library-wide fault, nothing removed`) — alertable, but deliberately not a rethrow, which would again skip the TV scan and autoSync for the tick. Previously one throwing movie rejected `scanMovies`' `Promise.all` — skipping the removal loop and, via `runGenerateList`'s sequential orchestration in `node/app.mjs`, the TV scan and autoSync for that tick — and one throwing show aborted every later show plus the removal loop under a single loop-wide `try`. The removal loop is also per-title isolated, logs at info (`scanner: removing N … no longer on disk`, with up to 20 names) so a departure leaves a SigNoz trail, and deletes the **title row last** (hashes → cooldown → [episode-retry rows] → row): the removal set is rebuilt from `movies`/`tv_shows` each tick, so a half-applied cascade keeps its row as the retry anchor instead of stranding orphan dependent rows (SI-P2 makes the cascade one transaction). Root-level failures (the media root itself unreadable) still propagate. One documented corner case of isolation: a title that throws before its identity claim is recorded leaves a cloned-sidecar copy, if one exists, to claim the id unopposed that tick; the original wins back on recovery and the clone is repointed (auditable via `previousIds`). Locked in by `node/tests/integration/scan-failed-not-absent.test.mjs` (16 cases: dangling-link and own-readdir failures, cascade failure via a SQLite trigger, root-level rejection; 5 of the original 11 fail against the pre-P0 scanners).
