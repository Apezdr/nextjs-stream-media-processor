...
- **First-scan skip (F-2) — still open.** Both scanners skip the immediate hash for brand-new titles: movies because `dirHashChanged = existingMovie && ...` is false when there is no existing row (Branch 1 widened the movie rehash gate to `dirHashChanged || needInfoRegeneration || needMetadataBackfill`, but both new terms are computed only for *existing* rows, so the brand-new-INSERT skip is deliberately kept); TV because the regeneration is gated `if (storedHash)`. A new title's first hash therefore comes only from the sweep (now content-aware, but still bounded by the 16-minute mtime lookback) or from the read-side fallbacks below. A new title can be absent from the bulk sync endpoint for ~15 minutes or more. Planned fix (Branch 4): `dirHashChanged || !existingMovie` and `if (dirHashChanged)` respectively — not yet implemented.
