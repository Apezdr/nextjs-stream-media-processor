...
| GET | `/api/identity/report` | webhook id or admin | Last reconcile report: written, conflicts (manual vs provider), provider-only, unmanaged, provider disagreements, errors (each list capped at 200). | 202 until the first reconcile has run; `{ enabled: false }` when no provider is configured. |
| POST | `/api/identity/reconcile` | webhook id or admin | Run a full reconcile now and return the report. | Shares one run with a concurrent tick. |
