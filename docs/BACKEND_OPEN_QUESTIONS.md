...
Thirteen theme-grouped branches, cut from `main` after the documentation branch lands. **Seven are unimplemented; Branches 1 (movies table schema parity), 2 (override & pristine-base helpers), and 3 (scanner cooldown & retry gating) are merged to `main`; Branches 4 (metadata-hash first-scan) and 5 (removal cascade) are implemented in this tree; Branch 13 was promoted into Track A and shipped in commit `4aeb30c`, retained in the numbering for traceability.** Three branches bundle items from different analysis lanes because the items edit the identical lines — building them separately guarantees merge conflicts or contradictory edits.
