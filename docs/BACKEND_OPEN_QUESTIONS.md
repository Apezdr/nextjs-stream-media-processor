...
| 2 | TMDB override & pristine-base helpers | G-2, G-3 | **Implemented (this branch)** — `hasMetadataOverrideKey()` shipped in `node/utils/tmdbConfig.mjs` (presence-based opt-in: a present `metadata` key, even `{}`, means override-managed) and both scanners' frozen-metadata retry gate consumes it; `generateForShow()`'s up-to-date branch now adopts the trusted id from the fresh `metadata.json` (persisted via the add-only ratchet) and falls back to the unvalidated name search only when the file carries no usable id. Override *application* (`applyMetadataOverrides`) keeps its current merge behavior until the pristine-base merge/revert semantics land. This row is removed post-merge. |
...
| 4 | Metadata-hash first-scan & bootstrap | F-2 | New titles get their content-aware hash on first scan — the remaining work. The row's other half, the bootstrap argument bug (`node/sqlite/metadataHashes.mjs` passed the db handle into `updateAllMovieHashes(db)` / `updateAllTVShowHashes(db)`'s `sinceTimestamp` parameter, so every `new Date(sinceTimestamp)` comparison was `Invalid Date` and the self-heal path processed zero rows), **was fixed alongside Branch 1 (this tree)**: `getMediaTypeHashes()` now calls both with no arguments for a true full-library bootstrap. |
