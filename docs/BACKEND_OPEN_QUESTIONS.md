...
The current contents come from the design-gap triage round of **2026-07-07**, which re-verified every claim against the code and produced: three hotfixes (shipped, see [Shipped](#shipped-track-a--commit-4aeb30c)), a 13-branch implementation plan ([Track B](#planned--track-b-branch-map); eight branches remain unimplemented — Branches 1, 2, and 3 are merged to `main`, Branch 4 (metadata-hash first-scan) is implemented in this tree, and Branch 13 was promoted into Track A and shipped, retained in the numbering for traceability), a full set of owner decisions on every open judgment call ([Decided 2026-07-07](#decided-2026-07-07)), and twelve behaviors confirmed correct and accepted as-is ([Accepted as documented](#accepted-as-documented)).
...
- **Must merge together (same lines):** Branch 3 (G-1a + R-1 + R-2 — constraint satisfied: implemented together in this branch), Branch 1 (G-5 + F-1 — constraint satisfied: implemented together in this branch), Branch 5 (R-4 + F-3). Do not split these into per-item PRs.
